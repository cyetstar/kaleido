...
def show_view():
    try:
        ...
        # 获取剧集信息
        tvshow = tmdb.get_tvshow(season['imdb_id'])
        # 补充剧集信息
        if season['season_number'] == 1:
            tvshow = douban.supply_tvshow(season, tvshow)
        elif tvshow.get('imdb_id') is not None:
            first_sesson = douban.get_season_by_imdb_id(tvshow.get('imdb_id'))
