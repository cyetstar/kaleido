...
def list_thread(page=1, catalog="719"):
    url = "https://bbs.eastgame.org/forumdisplay.php?fid=%s&page=%s" % (catalog, page)
    ...
    if result.status_code != 200:
        ...
        result = get_html(
            url, headers=_default_headers(), cookies=cookies)
    elements = _get_threads(result.text)
    threads = list(
        map(_deal_thread_item, filter(_filter_thread, elements)))
    return threads
...
def _get_threads(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    elements = html.xpath("//div[@id='threadlist']/form/table/tbody")
    return elements
...
def _deal_thread_item(element):
    title, url, type = _get_info_from_tbody(element)
    return {
        "thread_id": _get_id(element),
        "title": title,
        "url": url,
        "type": type,
        "imdb": _match_imdb(title),
