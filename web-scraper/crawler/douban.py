...
def search(keyword):
    url = (
        "https://search.douban.com/movie/subject_search?search_text={}&cat=1002".format(
            keyword
        )
    )
...
def get_doulist(douban_id):
    ...
    total = get_xpath_val(
        html, '//div[@class="doulist-filter"]/a/span/text()', "0"
    ).strip("()")
    thumb = get_xpath_val(html, "//div[@class='doulist-cover']/img/@src")
    about = "".join(html.xpath("//div[@class='doulist-about']/text()")).strip()
    updated = get_regex_text(
        get_xpath_val(html, "//div[@id='doulist-info']//span[@class='time']/text()"),
        r"(\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2})更新",
        1,
    )
    return {
        "douban_id": douban_id,
...
def get_doulist_movies(douban_id, start=0):
    ...
    return list(
        map(
            __deal_doulist_item,
            html.xpath('//*[@id="content"]//div[@class="doulist-item"]'),
        )
    )
...
def get_watched_movie(page=0):
    ...
    result = get_html(
        url,
        headers={**__default_headers(), **{"Cookie": douban.get("cookie")}},
        proxies=__proxies(),
    )
...
def __get_season_douban_id_list(html):
    try:
        return html.xpath("//*[@id='season']//option/@value")
    except Exception as e:
...
def __deal_doulist_item(elm):
    try:
        douban_id = get_regex_text(
            get_xpath_val(elm, './/div[@class="title"]/a/@href'), r"/(\d+)", 1
        )
...
def __fetch_twice(url, host="www.douban.com"):
    ...
    if (
        htmlcode is None
        or "页面不存在" in htmlcode
        or "条目不存在" in htmlcode
        or "豆瓣不收录" in htmlcode
    ):
        return None
...
def __get_year(html):
    try:
        result = get_xpath_text(html, "//*[@id='content']/h1/span[@class='year']")
        return result.strip("()")
    except Exception as e:
...
def __get_votes(html):
    try:
        return get_xpath_text(
            html, "//div[@id='interest_sectl']//span[@property='v:votes']"
        )
    except Exception as e:
    ...
    try:
        return list(
            filter(
                ...
                map(
                    __deal_actor_item,
                    html.xpath("//*[@id='info']//a[@rel='v:directedBy']"),
                ),
            )
        )
    except Exception as e:
    ...
    try:
        return list(
            filter(
                ...
                map(
                    __deal_actor_item,
                    html.xpath("//*[@id='info']//a[@rel='v:starring']"),
                ),
            )
        )
    except Exception as e:
...
def __deal_actor_item_new(elm):
    try:
        ...
        douban_id = get_regex_text(
            get_xpath_val(elm, 'div/span/a[@class="name"]/@href'), r".+/(\d+)/", 1
        )
        thumb = get_regex_text(
            get_xpath_val(elm, 'a/div[contains(@class, "avatar")]/@style'),
            r"url\((\S+)\)(?!,)",
            1,
        )
...
def __get_genres(html):
    try:
        return list(
            map(str, html.xpath("//*[@id='info']//span[@property='v:genre']/text()"))
        )
    except Exception as e:
...
def __get_release_date(html):
    try:
        ...
        for elm in elements:
            try:
                release_dates.append(re.search("\d{4}-\d{2}-\d{2}", str(elm)).group())
            except:
                pass
        release_dates.sort()
...
def __get_season_number(html):
    try:
        result = html.xpath("//*[@id='season']//option[@selected='selected']/text()")
        if len(result) > 0:
            # 根据下拉框确定季号
...
def __get_first_season_douban_id(html):
    try:
        result = html.xpath("//*[@id='season']//*[contains(text(),'1')]/@value")
        if len(result) > 0:
...
def __get_runtimes(html):
    try:
        return list(
            map(str, html.xpath("//*[@id='info']//span[@property='v:runtime']/text()"))
        )
    except Exception as e:
...
def __get_info(html):
    ...
    return {
        ...
        "countries": list(
            map(lambda x: x.strip(), filter(lambda x: x != "", countries))
        ),
        "languages": list(
            map(lambda x: x.strip(), filter(lambda x: x != "", languages))
        ),
...
def __get_doulists(html):
    try:
        return list(
            map(__deal_doulists_item, html.xpath("//*[@id='subject-doulist']/ul/li/a"))
        )
    except Exception as e:
...
def __default_headers(type=None):
    ...
    if type == "search":
        ...
        headers["Accept"] = (
            "image/jpg,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9"
        )
    else:
