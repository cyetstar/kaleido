...
public class ThreadController extends AbstractCrudController<ThreadDTO>{
    ...
    public CommonResult<ThreadViewResp> view(Long id) {
    ...
    public CommonResult<List<ExportColumn>> column() {
        List<ExportColumn> exportColumns = getColumns(ThreadExp.class);
        return CommonResult.success(exportColumns);
    }

    @GetMapping("export")
    @ApiOperation(value = "导出")
    public void export(ThreadPageReq req, String[] columns, PageParam pageParam, HttpServletResponse response) {
        String filename = "" + DateTimeUtils.now() + ".xlsx";
        super.export(req, columns, pageParam, filename, ThreadExp.class,
                    ThreadConvert.INSTANCE::convertToDTO, ThreadConvert.INSTANCE::convertToExp, response);
    }
