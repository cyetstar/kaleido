...
import cc.onelooker.kaleido.mexc.ExchangeInfo;
...
public class TradeSymbolController extends AbstractCrudController<TradeSymbolDTO> {
    ...
    public CommonResult<List<String>> fetch() {
        List<String> defaultSymbols = mexcApiService.listDefaultSymbols();
        tradeSymbolService.batchSave(defaultSymbols, KaleidoConstants.JYS_MEXC);
        return CommonResult.success(defaultSymbols);
    }
    ...
    public CommonResult<Boolean> fetchDetail(@RequestBody TradeSymbolFetchDetailReq req) {
        ...
        ExchangeInfo exchangeInfo = mexcApiService.getExchangeInfo(tradeSymbolDTO.getSpmc());
        ExchangeInfo.Symbol symbol = CollectionUtils.get(exchangeInfo.getSymbols(), 0);
        ...
        tradeSymbolDTO.setZdwtsl(new BigDecimal(symbol.getMaxQuoteAmount()));
        tradeSymbolDTO.setZxxdje(new BigDecimal(symbol.getQuoteAmountPrecision()));
        tradeSymbolDTO.setZxxdsl(new BigDecimal(symbol.getBaseSizePrecision()));
        tradeSymbolDTO.setMarkersxf(new BigDecimal(symbol.getMakerCommission()));
        tradeSymbolDTO.setTakersxf(new BigDecimal(symbol.getTakerCommission()));
